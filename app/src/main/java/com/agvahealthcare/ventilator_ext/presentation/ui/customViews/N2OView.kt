...
class N2OView @JvmOverloads constructor(
    context: Context?,
...
//        drawText(canvas)
    ...
    private fun drawWave(canvas: Canvas) {
        ...
        wavePaint!!.color = resources.getColor(R.color.blue)
        val wave = screenWidth / 4
        path!!.moveTo(startPoint.x.toFloat(), startPoint.y.toFloat())
        if (screenWidth < 100) amplitude = 10
        else amplitude = 20
        Log.i("wave",screenWidth.toString())
        for (i in 0..3) {
    ...
    private fun clipCircle(canvas: Canvas) {
        val circlePath = Path()
//        val recPath = Path()
//        recPath.addRect(screenHeignt.toFloat(),screenWidth.toFloat(),screenHeignt.toFloat(),screenWidth.toFloat(),Path.Direction.CW)
